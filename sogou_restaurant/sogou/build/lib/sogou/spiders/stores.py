...
class StoresSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        categories = ['中餐', '火锅', '烧烤', '西餐', '韩国料理', '日本料理', '快餐小吃', '甜品冷饮', '自助餐']
    ...
    def parse_tag(self, response):
        ...
        if stores:
            ...
            for store in stores:
                ...
                item['lng'] = x = store.get('bounds', {}).get('minx', '')
                item['lat'] = y = store.get('bounds', {}).get('miny', '')
                ...
                item['price'] = store.get('fields', {}).get('Map_price', {}).get('value', '')
                item['score'] = store.get('fields', {}).get('Map_score', {}).get('value', '')
    ...
    def parse_site(self, response):
        ...
        json_text = json.loads(json_str)
        item['specialfood'] = json_text.get('Map.DishTags', {}).get('value', '')
